import { ClientWithGetMinimumBalance, ClientWithPayer, ClientWithRpc, GetBalanceApi, pipe } from '@solana/kit';
import { addSelfPlanAndSendFunctions, SelfPlanAndSendFunctions } from '@solana/kit/program-client-core';

import { getCreateRecordInstructionPlan, getCreateRecordWithSeedInstructionPlan } from './createRecord';
import {
...
type PluginInstructionInput<TFn extends (client: never, input: never, config?: never) => unknown> = MakeOptional<
    Parameters<TFn>[1],
    'payer'
>;

type PluginInstructionConfig<TFn extends (client: never, input: never, config?: never) => unknown> = Parameters<TFn>[2];

type PluginInstructionReturn<TFn extends (client: never, input: never, config?: never) => unknown> = ReturnType<TFn> &
...
export function recordProgram() {
    return <T extends RecordPluginRequirements>(client: T) => {
        const withPayer = <TInput extends { payer?: RecordPluginRequirements['payer'] }>(input: TInput) =>
